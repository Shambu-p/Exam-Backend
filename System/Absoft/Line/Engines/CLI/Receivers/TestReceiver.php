...
use Absoft\Line\Engines\CLI\Commands\CommandFactory;
...
class TestReceiver
{
    ...
    public function startServer($arguments){
        ...
        if(isset($arguments["host"])){
            shell_exec("php -S ".$arguments["host"].":1111 -t ".$arguments["main_folder"]);
            print "line server started on ". $arguments["host"].":1111";
        }
        else{
            shell_exec("php -S localhost:1111 -t ".$arguments["main_folder"]);
            print "line server started on localhost:1111";
        }
