<?php
/**
 * Created by PhpStorm.
 * User: Abnet
 * Date: 2/5/2020
 * Time: 3:52 PM
 */



//print_r($_SESSION["_system"]);
//print Absoft\App\Files\Loader::cssAddress("bootstrap.min.css");


use Absoft\App\Files\Loader;
use Absoft\Line\FaultHandling\FaultHandler;


if(FaultHandler::checkError()){

    $er_array = $_SESSION["_system"]["error_handling"]["error"];

}else{
    $er_array["title"] = "Not Set";
    $er_array["description"] = "";
    $er_array["error_file"] = "no file";
}

$title = $er_array["title"];
$description = $er_array["description"];
$error_file = $er_array["error_file"];

FaultHandler::clearError();

?>

<html lang="en">
<head>
    <title>Error</title>
    <style><?php print Loader::cssAddress("bootstrap.min.css"); ?></style>
    <style><?php print Loader::cssAddress("main.css"); ?></style>
</head>
<body>
<br>
<div class="container shadow p-3 mb-5 rounded mycolor-white-level-2">
    <nav class="navbar navbar-light bg-light">
        <span class="navbar-brand mb-0 h1">
            <strong><i>Line</i></strong> <i>Framework</i>
        </span>
    </nav>
    <br>
    <div class="jumbotron bg-danger text-white">
        <h1 class="display-4">
            <?php

            if($title == E_NOTICE){

                print "ERROR NOTICE!!";

            }else if($title == E_WARNING){

                print "ERROR WARNING!!";

            }else if($title == E_ERROR){

                print "FATAL ERROR!!";

            }else{

                print $title;

            }

            ?>
        </h1>
        <hr class="my-4">
        <p class="lead">
            <?php

            print "Error occurred in file ".$error_file;
            ?>
        </p>
    </div>
    <div class="container text_color_red">
        <?php
        print $description;
        ?>
    </div>
</div>

<!--
<a class="btn btn-primary btn-lg" href="#" role="button">Learn more</a>
mycolor-white-level-2
<div class="shadow-sm p-3 mb-5 bg-white rounded">Small shadow</div>
<div class="shadow p-3 mb-5 bg-white rounded">Regular shadow</div>
-->

</body>
</html>
